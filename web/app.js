...
function initFilters(){const t=state.store.taxonomy;
  ...
  $('#status').innerHTML='<option value="">全部公开条目</option>'+t.hazardStatuses.map(option).join('');
...
function renderResults(){
  ...
  if(state.view==='hazards'){
    if(state.selectedHazard&&!state.store.searchIndex.some(x=>x.id===state.selectedHazard)){
      $$('.card').forEach(el=>el.onclick=()=>selectHazard(el.dataset.id));
      renderUnavailable(state.selectedHazard,'隐患');syncUrl();return;
    }
    if(!rows.some(x=>x.id===state.selectedHazard)) state.selectedHazard=rows[0]?.id||'';
    $$('.card').forEach(el=>el.onclick=()=>selectHazard(el.dataset.id));
    renderHazardDetail();
  }else{
    if(state.selectedLaw&&!state.store.lawIndex.some(x=>x.id===state.selectedLaw)){
      $$('.card').forEach(el=>el.onclick=()=>selectLaw(el.dataset.id));
      renderUnavailable(state.selectedLaw,'法规版本');syncUrl();return;
    }
...
function renderUnavailable(id,kind){$('#detail').innerHTML=`<div class="empty"><strong>此${esc(kind)}未在当前已核验库中发布</strong><p>链接编号：${esc(id)}</p><p>可能尚待核验、已撤下或编号有误。请从列表选择其他内容，或搜索已核验的依据。</p></div>`}

function hazardCard(r){return `<button class="card ${r.id===state.selectedHazard?'selected':''}" data-id="${esc(r.id)}" aria-pressed="${r.id===state.selectedHazard}"><div class="meta"><span title="完整记录ID：${esc(r.id)}">隐患编号 ${esc(displayHazardId(r.id))} · ${esc(r.category)}</span>${pill(r.status==='已核验'?r.mode:r.status)}</div><h3>${esc(r.title)}</h3><p>${esc(r.places.join(' / '))}</p><span class="arrow">↗</span></button>`}
