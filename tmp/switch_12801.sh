...
echo "== 7. 统一包 r26 =="
py -X utf8 tools/v4/build_unified_release.py --out source/releases/unified-v4-reviewed-20261001-r26 --as-of 2026-10-01 --data-version 2026.10.01.unified-v4-r26
py -X utf8 tools/v4/verify_unified_bundle.py --bundle source/releases/unified-v4-reviewed-20261001-r26

echo "== 8. site-selection -> r26 =="
py -X utf8 - << 'EOF'
import io, json
rel = json.load(io.open("source/releases/unified-v4-reviewed-20261001-r26/release.json", encoding="utf-8"))
rh = rel.get("releaseHash") or rel.get("release_hash")
p = "source/releases/site-selection.json"
d = json.load(io.open(p, encoding="utf-8"))
d["bundle"] = "unified-v4-reviewed-20261001-r26"
d["releaseHash"] = rh
...
echo "== 9. 铺根目录 =="
cp -r source/releases/unified-v4-reviewed-20261001-r26/* .
rm -rf _internal data/_internal

echo "== 10. 提交推送 =="
git add -A
git commit -m "release r26: switch GB/T 12801 citations to the 2025 edition (scheduled handover)"
git push origin data-verify-batch-003

echo "== 11. PR + merge =="
TOKEN=$(printf "protocol=https\nhost=github.com\n\n" | git credential fill 2>/dev/null | grep '^password=' | cut -d= -f2)
cat > tmp/pr26.json << 'EOF'
{"title": "release r26: switch GB/T 12801 citations to the 2025 edition (scheduled handover)", "head": "data-verify-batch-003", "base": "main", "body": "Scheduled 2026-10-01 switchover: 19 hazards re-linked to GB 12801-2025 clauses (old GB/T 12801-2008 links retired in r25). Gate six-phase PASS, audit PASS, bundle verify clean."}
EOF
curl -s -X POST -H "Authorization: token $TOKEN" -H "Accept: application/vnd.github+json" \
  https://api.github.com/repos/An-0726/safety-basis/pulls -d @tmp/pr26.json -o tmp/pr26_resp.json
