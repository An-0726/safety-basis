...
- 1264个新ID已全部写入 `knowledge/hazards/` 并生成处置结果。原始处置分类仍保留为：私有全文/条款候选388条、私有全文依据311条、法规目录命中475条、依据名称待核验83条、同名复核5条、重复/合并2条；这只是导入时分类，不代表最终状态。
- 当前状态为：1,250条新记录完成正式条款、证据、关联和审核绑定；2条同名记录已合并到已有正式隐患；11条以 `lifecycle=proposed` 的“待审核候选”状态进入当前公开发布包，可搜索、可查看，但不能直接作为正式依据。
