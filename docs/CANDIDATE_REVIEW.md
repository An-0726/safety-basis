...
4. **先查本地私有母库；本地没有、版本不全或原文不完整时，立即转查权威官方互联网来源，不得因本地缺失停止。**
5. 从已核验本地原件或官方网页/PDF中定位具体条、款、项/标准条号和完整逐字原文；官方网络来源满足身份、版本、原文、URL和适用性要求时，可直接作为正式 evidence，不要求先导入私有 SQLite；
6. 建立或复用 `knowledge/clauses/` 条款，保存逐字原文和来源；
7. 建立证据、条款审核、隐患审核和直接关联审核；
8. 只有完整 Gate 通过后，才把候选转为 `active`；
9. 重新构建、校验并更新 README 当前基线。

### 证据获取顺序

- 本地已有可靠原件：优先复用，避免重复下载和重复身份；
- 本地缺失或不足：主动联网查全国人大/国家法律法规数据库、中国政府网/国务院、发布机关或主管部门官网、国家标准全文公开系统/全国标准信息公共服务平台及地方政府/主管部门官网；
- 官方网页/PDF能够确认现行版本、精确条款、完整原文和适用范围：可以直接进入正式核验链；
- 普通搜索结果、AI摘要、OCR、Excel摘要、百科、培训网站、商业法规库和第三方转载：只能帮助定位，不能单独作为转正证据。
...
- 不把 AI 摘要、搜索摘要、Excel“依据条款要点”当成法规原文；同时也不把“本地母库没有”误判为“网上无可靠依据”或停止补证的理由；
- 不仅凭文件名或标准名称判断现行性；
