...
- 可发布隐患 **639 / 712**
- 合格关联 **806 / 839**（verified 806 / rejected 26 / superseded 7 / pending 0）

不可发布的 73 条 = 71 条 `superseded`（已合并或已拆分）+ 2 条 active 隐患因描述本身不成立暂不发布（见 2.D 与第 4 节），均不进入公开投影。
...
**GB 45067-2024《特种设备重大事故隐患判定准则》**：此前只登记了法规身份、**零条款**，而库内有 12 条隐患正是关于该判定（多条标题即标准原文）。本轮补入 4.1～4.10 十条判定准则（锅炉／压力容器／压力管道／充装／电梯／起重机械／客运索道／大型游乐设施／场车）并建立直接关联，《特种设备安全法》原关联降为 supporting。

**GB/T 47578-2026《压力容器定期检验方法》**：2026-12-01 才实施，只登记法规身份与 `upcoming` 版本，不录条款、不建关联；其第12章为转致条款（检验结论按 TSG 21 执行），本身不提供判定依据。
...
- 隐患搜索：639 条入库；关键词"消防培训"精确命中 H043，场所筛选"仓储场所"返回 14 条。
...
- 全量视图（712 条含未通过门禁者）移至 `data/_internal/`，与公开投影分离；公开目录只含 639 条可发布隐患及其依据。
...
因此不构成阻断项。它对应的 `activeHazardsWithoutQualifyingLink = 2`（本条与空压机房配电柜柜门那条）是有意保留的，不是遗漏。
