...
`verify` 是原始迁移对账工具，需要传入对应的旧资料快照；正常编辑母库后不能再要求正式内容等于旧来源。已有核心记录的 [Excel 编辑往返](EXCEL_EXCHANGE.md)、[候选入库与隐患合并](ADMISSION.md)、[证据核验及隔离发布](VERIFICATION_PUBLISH.md) 已实现。新增法规/条款/一般依据关联和身份冲突处理接口、生产切换仍待实现。
