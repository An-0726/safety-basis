...
截至 2026-09-09，已落地 [母库迁移](MASTER_MIGRATION.md)、[Excel 双向编辑](EXCEL_EXCHANGE.md)、[候选入库/检查项拆分/隐患合并](ADMISSION.md)、[证据核验与严格隔离发布](VERIFICATION_PUBLISH.md)。以下设计保留第一阶段检查基线；后续试跑只覆盖少量真实数据，生产构建尚未切换。新增法规/条款/一般依据关联、身份冲突处理等仍待实现。
...
第一阶段尚未取得原始 Excel。后续已取得用户提供的汇总工作簿，并于 2026-09-09 解析到隔离 staging，共 2776 行；只选 3 个检查项拆为 7 个模板。此数量不能写入模型容量、ID 逻辑或通用验收硬编码，试跑明细见核验发布文档。
...
| `source/releases/<releaseId>/` | 自动生成、仅已核验且审阅公开字段的快照 | 是；已有隔离样板，尚未用于部署 |
| `content/` | 迁移期原始规范化输入及 batch 历史 | 暂保留，切换后冻结归档 |
| `data/` | 网站 JSON、索引和分片，全部派生 | 是 |
| `tools/pipeline/` | Python 导入/整理/校验/发布命令 | 是 |
| `tools/build-data.mjs` | V2 运行格式编译器 | 是，已支持独立输入/输出目录供 release 适配器调用 |
...
核心实体使用稳定 ID 与 revision，操作时间及前后状态由审计记录保存；核验和合并记录追加保存，不用覆盖删除代替历史。
下表是正式母库设计；核心 DDL 与增量入库/核验表已落地。遗留法规临时身份和条款定位冲突保留，不能误认为已经完成全库去重。
...
早期架构轮用过 Luna 做独立审计和导入回归；2026-09-09 试跑轮的 Luna/high 启动返回 503 `auth_quota_exhausted`，没有产出。此次实际核验由主代理完成，未启动批量法规核验。

## 12. 第一阶段交付与第二阶段边界

后续实现进度见本文开头及 [入库与合并](ADMISSION.md)、[核验与发布](VERIFICATION_PUBLISH.md)。检查项拆分与新隐患 ID、隐患合并、少量实际终审和隔离发布已落地。新增法规/条款/一般关联、身份冲突处理、生产切换仍属后续步骤。下述“第一阶段”统计保留为当时验收记录。
...
第二阶段已实现正式母库 DDL、legacy 迁移、Excel 双向交换及隐患入库/合并/核验/发布闭环。先补齐剩余实体创建与冲突处理接口，再逐步扩展历史数据；不因试跑成功直接启动几千条核验。
