# Excel 母库编辑往返（exchange-v2）

SQLite 是唯一母库，Excel 是带版本的查看和编辑快照。网站 JSON 仍是构建产物。本工具已经实现 **导出 → 修改 → 差异提案 → 单事务提交**；Excel v2 只更新已有核心记录，不通过新增/删行表达入库和合并，并新增统一只读“字典”页。[入库/隐患合并](ADMISSION.md)、[法规目录/标签/别名](CATALOG.md) 及 [核验/隔离发布](VERIFICATION_PUBLISH.md) 使用独立提案接口。
...
| 字典 | 状态、核验结果和依据角色的稳定 code 与中文显示值 | 只读；统一来源为 `source/schemas/dictionaries.json` |
...
- 采用保守的整个母库状态指纹。导出后任何其他内容或核验记录发生变化，本轮工作簿和提案都视为过期，须重新导出并重新提出修改；v2 不自动合并并发编辑。
...
本工具不会写 `content/` 或根 `data/`。严格母库发布器逐层检查 hazard/law/version/clause/link 的当前 revision、依赖版本、核验证据和失效状态；输出为新隔离目录，不能仅凭 Excel 状态列发布。

通用原始 Excel 导入继续使用 `imports.py` 和 mapping 注册表，与这里的“编辑已有母库实体”分工不同；2776 条仍只是其中一个来源。新增实体、合并、法规身份修订和核验分别使用目录、入库、身份及核验提案接口。
