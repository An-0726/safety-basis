...
sub_once(
    "docs/PROJECT_STATE.md",
    r"> \*\*PHASE 10 已完成，不再重复发布验收。后续仅在有新的法规、版本、隐患、证据或维护需求时进入 PHASE 11 长期维护循环。\*\*\n\nPHASE 11 继续遵守现有治理边界：新证据先进入私有来源层并核身份/版本/效力；`knowledge/` 仍是唯一正式结构化事实源；候选必须经完整 Gate 才能转正式；`proposed` 不得直接进入公网；每次正式业务数据变化后再重新执行 Validate → Build → Pages → online acceptance。\s*\Z",
